...
func (p PostRepository) FindAll(post models.Post, keyword string) (*[]models.Post, int64, error) {
	...
	if keyword != "" {
		...
		queryBuider = queryBuider.Where(p.db.DB.Where("post.title LIKE ?", queryKeyword))
	}

	err := queryBuider.Where(post).Find(&posts).Count(&totalRows).Error
...
func (p PostRepository) First(post models.Post) (models.Post, error) {
	id := post.ID
	err := p.db.DB.First(&post, id).Error
	return post, err
}
